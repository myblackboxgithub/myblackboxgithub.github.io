...
if( Test-Path $manifestPath )
{
	...
	{
		#
		# Check if UAP project
		#
		$uap = $FALSE
		$uapns = New-Object Xml.XmlNamespaceManager $doc.NameTable
		$uapns.AddNamespace( "ns", "http://schemas.microsoft.com/appx/manifest/foundation/windows10" )
		$uapNode = $doc.SelectSingleNode("/ns:Package/ns:Dependencies/ns:TargetDeviceFamily[@Name='Windows.Universal']", $uapns)
		if( $uapNode -ne $null )
		{
			$uap = $TRUE
			Write-Host "Universal project"
		}
		...
		else
		{
			...
			$ns.AddNamespace( "m2ns", "http://schemas.microsoft.com/appx/2013/manifest" )
		}
		...
		#
		# Set the correct namespace for hid capabilities
		#
		$hidNs
		if( $uap )
		{
			$hidNs = $ns.LookupNamespace("ns");
		}
		else
		{
			$hidNs = $ns.LookupNamespace("m2ns");
		}
		
		#
		# Set namespace prefix, empty for UAP, "m2:" for 8.1 projects
		#
		$nsCreatePrefix = ""
		$nsSelectPrefix = "ns:"
		if(!$uap)
		{
			Write-Host "Add prefix"
			$nsCreatePrefix = "m2:"
			$nsSelectPrefix = "m2ns:"
		}
		...
		#	
		$hidelement = $capsnode.SelectSingleNode(".//" + $nsSelectPrefix + "DeviceCapability[@Name = 'humaninterfacedevice']", $ns)
		
		if( $hidelement -eq $null )
		{
			Write-Host "Adding HID node"
			$hidelement = $doc.CreateElement($nsCreatePrefix + "DeviceCapability", $hidNs)
		...
		#	
		$hiddeviceelement = $hidelement.SelectSingleNode(".//" + $nsSelectPrefix + "Device[@Id = 'vidpid:0421 0716']", $ns)
		if( $hiddeviceelement -eq $null )
		{
			Write-Host "Adding HID device node"
			$hiddeviceelement = $doc.CreateElement($nsCreatePrefix + "Device", $hidNs)
		...
		#	
		$page1element =  $hiddeviceelement.SelectSingleNode(".//" + $nsSelectPrefix + "Function[@Type = 'usage:ffaa 0001']", $ns)
		if( $page1element -eq $null )
		{
			Write-Host "Adding usage page 1 element"
			$page1element = $doc.CreateElement($nsCreatePrefix + "Function", $hidNs)
		...
		$page2element =  $hiddeviceelement.SelectSingleNode(".//" + $nsSelectPrefix + "Function[@Type = 'usage:ffee 0001']", $ns)
		if( $page2element -eq $null )
		{
			Write-Host "Adding usage page 2 element"
			$page2element = $doc.CreateElement($nsCreatePrefix + "Function", $hidNs)
		...
		$page3element =  $hiddeviceelement.SelectSingleNode(".//" + $nsSelectPrefix + "Function[@Type = 'usage:ffee 0002']", $ns)
		if( $page3element -eq $null )
		{
			Write-Host "Adding usage page 3 element"
			$page3element = $doc.CreateElement($nsCreatePrefix + "Function", $hidNs)
		...
		$page4element =  $hiddeviceelement.SelectSingleNode(".//" + $nsSelectPrefix + "Function[@Type = 'usage:ffee 0003']", $ns)
		if( $page4element -eq $null )
		{
			Write-Host "Adding usage page 4 element"
			$page4element = $doc.CreateElement($nsCreatePrefix + "Function", $hidNs)
		...
		$page5element =  $hiddeviceelement.SelectSingleNode(".//" + $nsSelectPrefix + "Function[@Type = 'usage:ffee 0004']", $ns)
		if( $page5element -eq $null )
		{
			Write-Host "Adding usage page 5 element"
			$page5element = $doc.CreateElement($nsCreatePrefix + "Function", $hidNs)
